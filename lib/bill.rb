...
class Bill
  ...
  def actual_price
    stored_items.each do|item, values|
      ...
      @total_price +=  item_details.unit_price * items_with_qty[item]
    end
    @total_price
  end
  ...
  def each_item_price(item)
    item_details = stored_items[item]
      if item_details.sale_qty != 0 && (items_with_qty[item] >= item_details.sale_qty)
        @item_sale_price =  (item_details.sale_price * (items_with_qty[item] / item_details.sale_qty)) + (item_details.unit_price * (items_with_qty[item] % item_details.sale_qty))
        total_sale_price(@item_sale_price)
        @item_sale_price
      else
        @item_sale_price = item_details.unit_price * items_with_qty[item]
        total_sale_price(@item_sale_price)
        @item_sale_price
      end
  end
